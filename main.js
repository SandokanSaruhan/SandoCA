$(document).ready(function(){
  ...
  arrows: true,
	...
	//Admin Sales
	/*
	carts_checkout_admin()
	function carts_checkout_admin()
	{
		$.ajax({
			url: 'action.php',
			method: 'POST',
			data: {carts_checkout_admin:1},
			success: function(data){
				$('#cartdetail_admin').html(data);
			}
		})
	}
	*/
	...
	$('body').delegate('.remove','click',function(e){
		...
		$.ajax({
			...
			success: function(data){
				$('#cart_msg').html(data);
				cart_checkout();
				//carts_checkout_admin(); //Admin Sales
			}
		})
	})
	...
	$('body').delegate('#price_sort','click',function(e){
		...
		$('#rand_sort').css('color','black');
		$('#range_sort').css('color','black');
		$.ajax({
			url: "action.php",
			method: "POST",
	...
	//Sort Func
	$('body').delegate('#rand_sort','click',function(e){
		e.preventDefault();
		$(this).css('color','red');
		$('#pop_sort').css('color','black');
		...
		$.ajax({
			url: "action.php",
			method: "POST",
			data: {getProduct:1,rand_sorted:1},
			success: function(data){
				$('#get_product').html(data);
			}
		})
	...
	//Sort Func
	$('body').delegate('#range_sort','click',function(e){
		e.preventDefault();
		$(this).css('color','red');
		$('#pop_sort').css('color','black');
		$('#price_sort').css('color','black');
		$('#rand_sort').css('color','black');
		 ...
		 var sliderValue = $('#price-range').val();

    	// Now you can use the sliderValue variable for further processing
    	console.log("Slider value: " + sliderValue);
    
		$.ajax({
			url: "action.php",
			method: "POST",
			data: {getProduct:1,range_sorted:1,maxData:sliderValue},
			//data: {getProduct:1,range_sorted:1},
			success: function(data){
				$('#get_product').html(data);
			}
		})
